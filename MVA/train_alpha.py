...
def load_data(fname, varlist, isSignal=True,channel='emu',lumi=41500):
    ...
    if channel=='emu':w = np.hstack([data_arrays[dataset]['weight'].value[((data_arrays[dataset]['region'].value=='SR2')|(data_arrays[dataset]['region'].value=='SR'))&(data_arrays[dataset]['lepflav'].value=='emu')]*genwei[dataset] for dataset in data_arrays.keys()])
    else :w = np.hstack([data_arrays[dataset]['weight'].value[((data_arrays[dataset]['region'].value=='SR2')|(data_arrays[dataset]['region'].value=='SR'))&(data_arrays[dataset]['lepflav'].value!='emu')]*genwei[dataset] for dataset in data_arrays.keys()])
    # Convert inputs to format readable by machine learning tools    
    if channel =='emu' : x = np.vstack([np.hstack([data_arrays[dataset][var].value[((data_arrays[dataset]['region'].value=='SR2')|(data_arrays[dataset]['region'].value=='SR'))&(data_arrays[dataset]['lepflav'].value=='emu')] for dataset in data_arrays.keys()]) for var in varlist]).T    
    else: x = np.vstack([np.hstack([data_arrays[dataset][var].value[((data_arrays[dataset]['region'].value=='SR2')|(data_arrays[dataset]['region'].value=='SR'))&(data_arrays[dataset]['lepflav'].value!='emu')] for dataset in data_arrays.keys()]) for var in varlist]).T    

    # Create labels
    if isSignal : y=np.ones(x.shape[0],dtype=int)
    else: y = np.zeros(x.shape[0],dtype=int)
    if isSignal : w = w*1000
    return x,y,w
...
if __name__ == "__main__":
    ...
    # Load data
    varlist = ["ll_pt","lll1_dr","lll2_dr","llc_dr","lep1_pt","lep2_pt","ll_mass","MET_pt","jetflav_pt","l1met_dphi","l2met_dphi","cW1_dphi","mT1","mT2","jetflav_btagDeepFlavCvL","jetflav_btagDeepFlavCvB"]
    bkgx, bkgy, bkgw = load_data('hists_HWW2l2nu_mcbkg_UL17arrays.coffea',varlist,False,args.channel)
    sigx, sigy, sigw = load_data('hists_HWW2l2nu_signal_UL17newdata.coffea',varlist,True,args.channel)
    fig,ax=plt.subplots()
    ...
    focal_params = {'imbalance_alpha':[1.5,2.0,2.5,3.0,4.0]}
   
    fix_params={'eval_metric':[ "logloss","auc"],
                'max_depth': 3,
                }
    clf = imb_xgb(fix_params,special_objective='weighted',num_round=500)
    bdt = GridSearchCV(clf, param_grid=focal_params, n_jobs=1)
    model = bdt.fit(x,y,sample_weight=np.abs(w))    
    best_model = model.best_estimator_

    best_model.boosting_model.save_model(f"SR_{args.channel}_scanalpha_{args.year}_{args.version}.json")
    ### Save plots
    fig,ax=plt.subplots()
    ...
    bin_counts2, bin_edges2, patches2 = plt.hist(bkgpred_test, bins=np.linspace(0,1,20))
    fig,ax = plt.subplots()
    
    ax=plt.hist(sigpred_train,bins=np.linspace(0,1,20),
                histtype='step',color='blue',facecolor=None,label='signal:train')
    ax=plt.hist(bkgpred_train,bins=np.linspace(0,1,20),
                histtype='step',color='red',label='background:train')
    
    bin_centres = (bin_edges[:-1] + bin_edges[1:]) / 2
    bin_centres2 = (bin_edges2[:-1] + bin_edges2[1:]) / 2
    from sklearn.metrics import roc_curve,auc,accuracy_score
    ax=plt.errorbar(x=bin_centres, y=bin_counts,yerr=np.zeros_like(bin_counts), fmt='o', capsize=2,color='blue',label='signal:test')
    ax=plt.errorbar(x=bin_centres2, y=bin_counts2,yerr=np.zeros_like(bin_counts2), fmt='o', capsize=2,color='red',label='background:test')
    ...
    plt.plot(fpr2, tpr2, color='tab:blue',
            lw=lw, label='Train:ROC curve (area = %0.2f)' % roc_auc2)
