...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        # global_cut = (leppair.lep1.pt>25) & (leppair.lep1.charge+leppair.lep2.charge==0) #& (events.MET.pt>20)  & ((abs(ll_cand.mass-91.)<15) | (abs(events.MET.pt-91.)<15)) & (abs(met.delta_phi(tkmet))<0.5)
        global_cut = (
            (leppair.lep1.pt > 25)
        ...
        sr_cut = (
            (abs(met.delta_phi(leppair.lep1)) > 0.2)
            & (abs(met.delta_phi(leppair.lep2)) > 0.2)
        ...
        selection.add("SR", ak.to_numpy(ak.any(global_cut, axis=-1)))
        # &sr_cut&zmass_cut
        ...
        seljet = (
            (corr_jet.pt > 20)
            & (abs(corr_jet.eta) <= 2.4)
            & ((corr_jet.puId > 6) | (corr_jet.pt > 50))
            & (corr_jet.jetId > 5)
        )  # &ak.all(corr_jet.metric_table(leppair.lep1)>0.4,axis=2)&ak.all(corr_jet.metric_table(leppair.lep2)>0.4,axis=2)
        selection.add(
            "jetsel",
        ...
        sel_jet = eventcsv_jet[
            ...
            & ((eventcsv_jet.puId > 0) | (eventcsv_jet.pt > 50))
        ...
        sel_jetflav = eventflav_jet[
            ...
            & ((eventflav_jet.puId > 0) | (eventflav_jet.pt > 50))
        ...
        sel_jetpt = eventpt_jet[
            ...
            & ((eventpt_jet.puId > 0) | (eventpt_jet.pt > 50))
            & (eventpt_jet.jetId > 5)
        ...
        for histname, h in output.items():
            for ch in lepflav:
                cut = selection.all(
                    "lepsel", "SR", "lumi", "metfilter", ch, "trigger_%s" % (ch)
                )  # ,'jetsel')

                ll_cands = ak.mask(ll_cand, global_cut)  # &zmass_cut&global_cut)
