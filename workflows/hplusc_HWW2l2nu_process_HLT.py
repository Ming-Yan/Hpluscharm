...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process_shift(self, events, shift_name):
        ...
        output = {
            "hist_no": hist.Hist(
                hist.axis.Variable(
                    [12, 16, 20, 25, 30, 35, 45, 60, 80, 100, 200],
                    name="pt_e",
                    label=" $p_{T}^{e}$ [GeV]",
                ),
                hist.axis.Variable(
            ...
            "hist_MET": hist.Hist(
                ...
                hist.axis.Variable(
                    [0, 0.5, 1.0, 1.442, 1.566, 2.5], name="abseta_e", label=" e [GeV]"
                ),
                hist.axis.Variable(
                    [0, 0.5, 1.0, 1.442, 1.566, 2.5],
                    name="abseta_mu",
        ...
        )  # & (abs(event_e.dxy) < 0.05) & (abs(event_e.dz) < 0.1)
        # elesel =  (event_e.pt>13)&((abs(event_e.eta) < 1.4442)|((abs(event_e.eta) > 1.566)&(abs(event_e.eta)<2.5))) &  (event_e.cutBased >=4)
        ...
        # lepton_cut=(ll_cand.lep2.pt > 13)&(ll_cand.lep1.pt > 25) & (make_p4(ll_cand.lep1).delta_r(make_p4(ll_cand.lep2)) > 0.4) & (ll_cand.lep1.charge + ll_cand.lep2.charge == 0)
        ...
        baseline = (
            ...
            & metfilter
            & (met.pt > met_cut)
            & (ak.sum(lepton_cut & dilep_cut, axis=-1) > 0)
            & (ak.num(event_e) > 0)
            & (ak.num(event_mu) > 0)
        )
        cuts = {
            ...
            "nomtjet": baseline & metsel & (ak.sum(jet_sel, axis=-1) >= 1),
            "nomtcjet": baseline & metsel & (ak.sum(jet_sel & cjetsel, axis=-1) >= 1),
        ...
        for r in cuts:
            ...
            lep1cut = llcut.lep1
            lep2cut = llcut.lep2
            print(ak.type(lep_HLT), ak.type(lep1cut.pt))
            ...
            if isRealData:
                ...
                weights.add(
                    "genweight", events[cut].genWeight / abs(events[cut].genWeight)
                )
                weights.add(
                    "L1prefireweight",
