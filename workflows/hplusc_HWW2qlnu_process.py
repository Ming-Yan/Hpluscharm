...
class NanoProcessor(processor.ProcessorABC):
    ...
    def __init__(self, year="2017"):
        ...
        _hist_event_dict = {
            ...
            "nbj": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, nbjet_axis
            ),
            "ncj": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, ncjet_axis
            ),
            "hj_dr": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, dr_axis
            ),
            "MET_sumEt": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, sumEt_axis
            ),
            "MET_significance": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, signi_axis
            ),
            "MET_covXX": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, covXX_axis
            ),
            "MET_covXY": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, covXY_axis
            ),
            "MET_covYY": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, covYY_axis
            ),
            "MET_phi": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, phi_axis
            ),
            "MET_pt": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, pt_axis
            ),
            "mT": hist.Hist("Counts", dataset_axis, lepflav_axis, region_axis, mt_axis),
            "mTh": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, mt_axis
            ),
            "mjjl": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, mt_axis
            ),
        ...
        for i in objects:
            if "jet" in i:
                ...
                _hist_event_dict["%s_eta" % (i)] = hist.Hist(
                    "Counts", dataset_axis, lepflav_axis, region_axis, eta_axis
                )
                _hist_event_dict["%s_phi" % (i)] = hist.Hist(
                    "Counts", dataset_axis, lepflav_axis, region_axis, phi_axis
                )
                _hist_event_dict["%s_mass" % (i)] = hist.Hist(
                    "Counts", dataset_axis, lepflav_axis, region_axis, mass_axis
                )
    ...
    def process(self, events):
        ...
        if isRealData:
            ...
            output["cutflow"][dataset]["all"] += ak.sum(
                abs(events.genWeight) / abs(events.genWeight)
            )
        ...
        musel = (
            ...
            & (event_mu.mvaId >= 3)
            & (event_mu.pfRelIso04_all < 0.35)
            & (abs(event_mu.dxy) < 0.5)
            & (abs(event_mu.dz) < 1)
        )
        ...
        elesel = (
            ((event_e.pt > 30) & (abs(event_e.eta) < 2.5))
        ...
        event_jet = corr_jet[
            ak.argsort(corr_jet.btagDeepFlavCvL, axis=1, ascending=False)
        ]
        ...
        req_dr = ak.any(
            (make_p4(jj_cand.jet1).delta_r(good_leptons) > 0.4), axis=-1
        ) & ak.any((make_p4(jj_cand.jet2).delta_r(good_leptons) > 0.4), axis=-1)
        req_wqqmass = ak.any((jj_cand.mass < 116) & (jj_cand.mass > 46), axis=-1)

        req_mT = mT(make_p4(good_leptons), met) > 60
        req_dphi = ak.any(met.delta_phi(jj_cand) < np.pi / 2.0, axis=-1)
        ...
        output["cutflow"][dataset]["lep candidates"] += ak.sum(
            (njet >= 3) & req_wqqmass & (nmu + nele >= 1)
        )
        output["cutflow"][dataset]["MET"] += ak.sum(
            (njet >= 3) & req_wqqmass & (nmu + nele >= 1) & req_global
        )
        output["cutflow"][dataset]["mT"] += ak.sum(
            (njet >= 3) & req_wqqmass & (nmu + nele >= 1) & req_global & req_mT
        )
        output["cutflow"][dataset]["dr"] += ak.sum(
            (njet >= 3) & req_wqqmass & (nmu + nele >= 1) & req_global & req_mT & req_dr
        )
        output["cutflow"][dataset]["dphi"] += ak.sum(
            (njet >= 3)
            & req_wqqmass
            & (nmu + nele >= 1)
            & req_global
        ...
        output["cutflow"][dataset]["c jets"] += ak.sum(
            (njet >= 3)
            & req_wqqmass
            & (nmu + nele >= 1)
            & req_global
        ...
        output["cutflow"][dataset]["electron eff"] += ak.sum(
            req_wqqmass
            & req_sr
            & req_global
            & (nele == 1)
        ...
        output["cutflow"][dataset]["muon eff"] += ak.sum(
            req_wqqmass
            & req_sr
            & req_global
            & (nmu == 1)
        ...
        for histname, h in output.items():
            for ch in lepflav:
                for r in region:
                    ...
                    if not isRealData:
                        if ch == "ee":
                            lepsf = eleSFs(lepcut, self._year, self._corr)
                        elif ch == "mumu":
                            lepsf = muSFs(lepcut, self._year, self._corr)
                    else:
                    ...
                    if "cjet_" in histname:
                        ...
                        if isRealData:
                            flavor = ak.zeros_like(normalize(sel_cjet["pt"], cut))
                        else:
                            flavor = normalize(
                        ...
                        if isRealData:
                            flavor = ak.zeros_like(flatten(jet1cut["pt"]))
                        else:
                            flavor = flatten(
                                (
                        ...
                        if isRealData:
                            flavor = ak.zeros_like(flatten(jet2cut["pt"]))
                        else:
                            flavor = flatten(
                                (
                        ...
                        fields = {
                            l: ak.fill_none(
                                lepcut[histname.replace("lep_", "")], np.nan
                            )
                            for l in h.fields
                            if l in dir(lepcut)
                        }
                        ...
                        output["mT"].fill(
                            ...
                            lepflav=ch,
                            region=r,
                            mt=ak.fill_none(mT(lepcut, met[cut]), np.nan),
                            weight=weights.weight()[cut] * lepsf,
                        )
                        output["mTh"].fill(
                            dataset=dataset,
                            lepflav=ch,
                            region=r,
                            mt=ak.fill_none((met[cut] + lepcut + jjcut).mass, np.nan),
                            weight=weights.weight()[cut] * lepsf,
                        )
                        output["mjjl"].fill(
                            dataset=dataset,
                            lepflav=ch,
                            region=r,
                            mt=ak.fill_none((lepcut + jjcut).mass, np.nan),
                            weight=weights.weight()[cut] * lepsf,
                        )
                        output["dphi_ww"].fill(
                            dataset=dataset,
                            lepflav=ch,
                            region=r,
                            phi=ak.fill_none(met[cut].delta_phi(jjcut), np.nan),
                            weight=weights.weight()[cut] * lepsf,
                        )
                        output["dphi_lep"].fill(
                            dataset=dataset,
                            lepflav=ch,
                            region=r,
                            phi=ak.fill_none(met[cut].delta_phi(lepcut), np.nan),
                            weight=weights.weight()[cut] * lepsf,
                        )
