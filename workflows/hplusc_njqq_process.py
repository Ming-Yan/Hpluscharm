...
class NanoProcessor(processor.ProcessorABC):
    ...
    def __init__(self, year="2017", version="test"):
        ...
        _hist_event_dict = {
            "nj": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, njet_axis
            ),
            "njnosel": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, njet_axis
            ),
            "nele": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, nalep_axis
            ),
            "nmu": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, nalep_axis
            ),
            "hc_dr": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, njet_axis, dr_axis
            ),
            "zs_dr": hist.Hist(
                "Counts", dataset_axis, lepflav_axis, region_axis, njet_axis, dr_axis
            ),
    ...
    def process(self, events):
        ...
        selection.add(
            "global", ak.to_numpy(ak.any(ak.any(req_global, axis=-1), axis=-1))
        )
        selection.add(
            "dr", ak.to_numpy(ak.any(ak.any(req_global & req_dr, axis=-1), axis=-1))
        )
        selection.add(
            "llmass",
            ak.to_numpy(
                ak.any(ak.any(req_zllmass & req_global & req_dr, axis=-1), axis=-1)
            ),
        )
        selection.add(
            "qqmass",
            ak.to_numpy(
                ak.any(
                    ak.any(req_zllmass & req_global & req_zqqmass & req_dr, axis=-1),
                    axis=-1,
                )
            ),
        )
        ...
        srmasks = {
            "dr": req_dr & req_global,
            "llmass": req_dr & req_zllmass & req_global,
            "qqmass": req_dr & req_zllmass & req_global & req_zqqmass,
            "global": req_global,
            "hmass": req_zllmass & req_global & req_zqqmass & req_hmass & req_dr,
        }
        for histname, h in output.items():
            for ch in lepflav:
                for r in regions:
                    cut = selection.all(
                        "lepsel", r, "lumi", "metfilter", ch, "trigger_%s" % (ch)
                    )
                    ...
                    if ak.count(hcut.mass) > 0:
                        hcut = hcut[
                            ak.argsort(abs(hcut.ll_cands.mass - 91.18), axis=-1)
                        ]
                    ...
                    if "cjet_" in histname:
                        ...
                        fields = {
                            l: ak.fill_none(
                                jet1cut[histname.replace("jet1_", "")], np.nan
                            )
                            for l in h.fields
                            if l in dir(jet1cut)
                        }
                        ...
                        fields = {
                            l: ak.fill_none(
                                jet2cut[histname.replace("jet2_", "")], np.nan
                            )
                            for l in h.fields
                            if l in dir(jet2cut)
                        }
                        ...
                        fields = {
                            l: ak.fill_none(
                                lep1cut[histname.replace("lep1_", "")], np.nan
                            )
                            for l in h.fields
                            if l in dir(lep1cut)
                        }
                        ...
                        fields = {
                            l: ak.fill_none(
                                lep2cut[histname.replace("lep2_", "")], np.nan
                            )
                            for l in h.fields
                            if l in dir(lep2cut)
                        }
                        ...
                        fields = {
                            l: ak.fill_none(llcut[histname.replace("ll_", "")], np.nan)
                            for l in h.fields
                            if l in dir(llcut)
                        }
                        h.fill(
                        ...
                        fields = {
                            l: ak.fill_none(jjcut[histname.replace("jj_", "")], np.nan)
                            for l in h.fields
                            if l in dir(jjcut)
                        }
                        h.fill(
                        ...
                        fields = {
                            l: ak.fill_none(
                                hcut[histname.replace("higgs_", "")], np.nan
                            )
                            for l in h.fields
                            if l in dir(hcut)
                        }
                        ...
                        output["njnosel"].fill(
                            ...
                            region=r,
                            nj=ak.fill_none(
                                ak.count(events[cut].Jet.pt, axis=-1), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["nele"].fill(
                        ...
                        output["hc_dr"].fill(
                            ...
                            region=r,
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(hcut.delta_r(charmcut), np.nan),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["zs_dr"].fill(
                            dataset=dataset,
                            ...
                            region=r,
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(llcut.delta_r(jjcut), np.nan),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["j1j2_dr"].fill(
                            dataset=dataset,
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(jet1cut).delta_r(make_p4(jet2cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["l1l2_dr"].fill(
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(lep1cut).delta_r(make_p4(lep2cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["l1j1_dr"].fill(
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(lep1cut).delta_r(make_p4(jet1cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["l1j2_dr"].fill(
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(lep1cut).delta_r(make_p4(jet2cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["l2j1_dr"].fill(
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(jet1cut).delta_r(make_p4(lep2cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["l2j2_dr"].fill(
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(lep2cut).delta_r(make_p4(jet2cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["jjc_dr"].fill(
                            ...
                            region=r,
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(jjcut.delta_r(charmcut), np.nan),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["cj_dr"].fill(
                            dataset=dataset,
                            ...
                            region=r,
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(make_p4(jet1cut).delta_r(charmcut), np.nan),
                            weight=weights.weight()[cut] * sf,
                        )
                        output["lc_dr"].fill(
                            dataset=dataset,
                            ...
                            nj=normalize(njet, cut),
                            dr=ak.fill_none(
                                make_p4(charmcut).delta_r(make_p4(lep1cut)), np.nan
                            ),
                            weight=weights.weight()[cut] * sf,
                        )
