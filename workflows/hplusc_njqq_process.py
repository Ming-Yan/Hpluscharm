...
class NanoProcessor(processor.ProcessorABC):
    ...
    def __init__(self, year="2017",version="test"):    
        ...
        region_axis =  hist.Cat("region", "cuts")
        # Events
        ...
        _hist_event_dict = {
                ...
                'njnosel'  : hist.Hist("Counts", dataset_axis,  lepflav_axis,region_axis, njet_axis),
                ...
                'hc_dr'  : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'zs_dr' : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'jjc_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'cj_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'l1l2_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,njet_axis,  dr_axis),
                'lc_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'l1j1_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'l1j2_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'l2j1_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'l2j2_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dr_axis),
                'j1j2_dr': hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,njet_axis, dr_axis),
                # 'costheta_ll'  : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, costheta_axis),
                # 'costheta_qq'  : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, costheta_axis),
                # 'costheta_pz'  : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, costheta_axis),
                # 'phi_zz'  : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, phi_axis),
                # 'phi_lq'  : hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, phi_axis),
            }
        objects=['cjet','lep1','lep2','jet1','jet2','jj','ll','higgs']
        
        for i in objects:
            if  'jet' in i: 
                _hist_event_dict["%s_pt" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, flav_axis,njet_axis,  pt_axis)
                _hist_event_dict["%s_eta" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,flav_axis,njet_axis,  eta_axis)
                _hist_event_dict["%s_phi" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,flav_axis,njet_axis,  phi_axis)
                _hist_event_dict["%s_mass" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,flav_axis, njet_axis, mass_axis)
            else:
                _hist_event_dict["%s_pt" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis ,njet_axis, pt_axis)
                _hist_event_dict["%s_eta" %(i)]=hist.Hist("Counts", dataset_axis,lepflav_axis,region_axis, njet_axis, eta_axis)
                _hist_event_dict["%s_phi" %(i)]=hist.Hist("Counts", dataset_axis,  lepflav_axis,region_axis, njet_axis, phi_axis)
                if i =='ll' : _hist_event_dict["%s_mass" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,njet_axis,  llmass_axis)
                else:_hist_event_dict["%s_mass" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, mass_axis)
                if 'lep' in i: 
                    _hist_event_dict["%s_pfRelIso03_all" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,njet_axis, region_axis, iso_axis)
                    _hist_event_dict["%s_dxy" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis, njet_axis, dxy_axis)
                    _hist_event_dict["%s_dz" %(i)]=hist.Hist("Counts", dataset_axis, lepflav_axis,region_axis,njet_axis,  dz_axis)

            
        
        for disc, axis in zip(disc_list,btag_axes):
            _hist_event_dict["cjet_%s" %(disc)] = hist.Hist("Counts", dataset_axis,lepflav_axis,region_axis,flav_axis, njet_axis, axis)
            _hist_event_dict["jet1_%s" %(disc)] = hist.Hist("Counts", dataset_axis,lepflav_axis,region_axis,flav_axis, njet_axis, axis)
            _hist_event_dict["jet2_%s" %(disc)] = hist.Hist("Counts", dataset_axis,lepflav_axis,region_axis,flav_axis, njet_axis, axis)
    ...
    def process(self, events):
        ...
        jet_sel = (event_jet.pt > 20) & (abs(event_jet.eta) <= 2.4)&((event_jet.puId > 6)|(event_jet.pt>50)) &(event_jet.jetId>5) #&(ak.all(event_jet.metric_table(event_e)>0.4,axis=2))&(ak.all(event_jet.metric_table(event_mu)>0.4,axis=2))
        ...
        # if 'DY' in dataset :weights.add('njwei',nJet_corr(njet,self._corr))
        cjet = event_jet[:,0]
        ...
        seljet = (cjet.pt > 20) & (abs(cjet.eta) <= 2.4)&((cjet.puId > 6)|(cjet.pt>50)) &(cjet.jetId>5)&(ak.all(ak.all((cjet.delta_r(higgs_cand.ll_cands.lep1)>0.4)&(cjet.delta_r(higgs_cand.ll_cands.lep2)>0.4)&(cjet.delta_r(higgs_cand.jj_cands.jet1)>0.4)&(cjet.delta_r(higgs_cand.jj_cands.jet2)>0.4),axis=-1),axis=-1))
        ...
        selection.add('global',ak.to_numpy(ak.any(ak.any(req_global,axis=-1),axis=-1)))        
        selection.add('dr',ak.to_numpy(ak.any(ak.any(req_global&req_dr,axis=-1),axis=-1)))
        selection.add('llmass',ak.to_numpy(ak.any(ak.any(req_zllmass&req_global&req_dr,axis=-1),axis=-1)))
        selection.add('qqmass',ak.to_numpy(ak.any(ak.any(req_zllmass&req_global&req_zqqmass&req_dr,axis=-1),axis=-1)))
        selection.add('hmass',ak.to_numpy(ak.any(ak.any(req_zllmass&req_global&req_zqqmass&req_hmass&req_dr,axis=-1),axis=-1)))
        ...
        lepflav = ['ee','mumu']
        regions=['llmass','dr','qqmass','global','hmass']
        srmasks = {'dr':req_dr&req_global,'llmass':req_dr&req_zllmass&req_global,'qqmass':req_dr&req_zllmass&req_global&req_zqqmass,'global':req_global,'hmass':req_zllmass&req_global&req_zqqmass&req_hmass&req_dr}
        for histname, h in output.items():
            for ch in lepflav:
                for r in regions:
                    cut = selection.all('lepsel',r,'lumi','metfilter',ch,'trigger_%s'%(ch))
                    sr_mask = ak.mask(higgs_cand,srmasks[r])
                    ...
                    if 'cjet_' in histname:
                        ...
                        h.fill(dataset=dataset, lepflav =ch,region=r,flav=flavor, **fields,nj=normalize(njet,cut),weight=weights.weight()[cut]*sf)                  
                    elif 'jet1_' in histname:
                        fields = {l: ak.fill_none(jet1cut[histname.replace('jet1_','')],np.nan) for l in h.fields if l in dir(jet1cut)}
                        if isRealData:flavor= ak.zeros_like(sf)
                        else :flavor= ak.fill_none(jet1cut.hadronFlavour+1*((jet1cut.partonFlavour == 0 ) & (jet1cut.hadronFlavour==0)),np.nan)
                        h.fill(dataset=dataset, lepflav =ch,region=r,flav=flavor, **fields,nj=normalize(njet,cut),weight=weights.weight()[cut]*sf) 
                    elif 'jet2_' in histname:
                        fields = {l: ak.fill_none(jet2cut[histname.replace('jet2_','')],np.nan) for l in h.fields if l in dir(jet2cut)}
                        if isRealData:flavor= ak.zeros_like((weights.weight()[cut]))
                        else :flavor= ak.fill_none(jet2cut.hadronFlavour+1*((jet2cut.partonFlavour == 0 ) & (jet2cut.hadronFlavour==0)),np.nan)
                        h.fill(dataset=dataset, lepflav =ch,region=r,flav=flavor, **fields,nj=normalize(njet,cut),weight=weights.weight()[cut]*sf)    
                    elif 'lep1_' in histname:
                        fields = {l: ak.fill_none(lep1cut[histname.replace('lep1_','')],np.nan) for l in h.fields if l in dir(lep1cut)}
                        h.fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut), **fields,weight=weights.weight()[cut]*sf)
                    elif 'lep2_' in histname:
                        fields = {l: ak.fill_none(lep2cut[histname.replace('lep2_','')],np.nan) for l in h.fields if l in dir(lep2cut)}
                        h.fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut), **fields,weight=weights.weight()[cut]*sf)
                    elif 'll_' in histname:
                        fields = {l: ak.fill_none(llcut[histname.replace('ll_','')],np.nan) for l in h.fields if l in dir(llcut)}
                        h.fill(dataset=dataset, lepflav =ch,region=r,nj=normalize(njet,cut), **fields,weight=weights.weight()[cut]*sf)  
                    elif 'jj_' in histname:
                        fields = {l:  ak.fill_none(jjcut[histname.replace('jj_','')],np.nan) for l in h.fields if l in dir(jjcut)}
                        h.fill(dataset=dataset, lepflav =ch,region=r,nj=normalize(njet,cut), **fields,weight=weights.weight()[cut]*sf)
                    elif 'higgs_' in histname:
                        fields = {l:  ak.fill_none(hcut[histname.replace('higgs_','')],np.nan) for l in h.fields if l in dir(hcut)}
                        h.fill(dataset=dataset, lepflav =ch,region=r,nj=normalize(njet,cut), **fields,weight=weights.weight()[cut]*sf)  
                    else :

                        output['nj'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),weight=weights.weight()[cut]*sf)
                        output['njnosel'].fill(dataset=dataset,lepflav=ch,region=r,nj=ak.fill_none(ak.count(events[cut].Jet.pt,axis=-1),np.nan),weight=weights.weight()[cut]*sf)
                        output['nele'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),nalep=normalize(naele-nele,cut),weight=weights.weight()[cut]*sf)
                        output['nmu'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),nalep=normalize(namu-nmu,cut),weight=weights.weight()[cut]*sf)    
                        output['hc_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(hcut.delta_r(charmcut),np.nan),weight=weights.weight()[cut]*sf)                    
                        output['zs_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(llcut.delta_r(jjcut),np.nan),weight=weights.weight()[cut]*sf) 
                        output['j1j2_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(jet1cut).delta_r(make_p4(jet2cut)),np.nan),weight=weights.weight()[cut]*sf)
                        output['l1l2_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(lep1cut).delta_r(make_p4(lep2cut)),np.nan),weight=weights.weight()[cut]*sf)
                        output['l1j1_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(lep1cut).delta_r(make_p4(jet1cut)),np.nan),weight=weights.weight()[cut]*sf)
                        output['l1j2_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(lep1cut).delta_r(make_p4(jet2cut)),np.nan),weight=weights.weight()[cut]*sf)
                        output['l2j1_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(jet1cut).delta_r(make_p4(lep2cut)),np.nan),weight=weights.weight()[cut]*sf)
                        output['l2j2_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(lep2cut).delta_r(make_p4(jet2cut)),np.nan),weight=weights.weight()[cut]*sf)                    
                        output['jjc_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(jjcut.delta_r(charmcut),np.nan),weight=weights.weight()[cut]*sf)
                        output['cj_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(jet1cut).delta_r(charmcut),np.nan),weight=weights.weight()[cut]*sf)
                        output['lc_dr'].fill(dataset=dataset,lepflav=ch,region=r,nj=normalize(njet,cut),dr=ak.fill_none(make_p4(charmcut).delta_r(make_p4(lep1cut)),np.nan),weight=weights.weight()[cut]*sf)
                        ...
                        # output['costheta_pz'].fill(dataset=dataset,lepflav=ch,region=r,costheta=ak.fill_none(np.cos(ll_hCM.theta),np.nan),weight=weights.weight()[cut]*sf) 
                        # output['costheta_ll'].fill(dataset=dataset,lepflav=ch,region=r,costheta=ak.fill_none(np.cos(poslep_ZCM.theta),np.nan),weight=weights.weight()[cut]*sf) 
                        # output['costheta_pz'].fill(dataset=dataset,lepflav=ch,region=r,costheta=ak.fill_none(np.cos(jet_ZCM.theta),np.nan),weight=weights.weight()[cut]*sf) 
                        # output['phi_zz'].fill(dataset=dataset,lepflav=ch,region=r,phi=ak.fill_none(ll_hCM.phi,np.nan),weight=weights.weight()[cut]*sf) 
                        # output['phi_lq'].fill(dataset=dataset,lepflav=ch,region=r,phi=ak.fill_none(jet_ZCM.delta_phi(poslep_ZCM),np.nan),weight=weights.weight()[cut]*sf)
