...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        # 
        # event_e = ak.pad_none(events.Electron, 4, axis=-1) 
        # event_mu = ak.pad_none(events.Muon, 4, axis=-1) 
        # req_elept =  (event_e[:,0].pt>20)&(event_e[:,1].pt>10)
        # req_mupt = (event_mu[:,0].pt>20)&(event_mu[:,1].pt>10)
        ...
        ###########
        event_jet = events.Jet[(events.Jet.pt > 25) & (abs(events.Jet.eta) <= 2.4)&((events.Jet.puId > 0)|(events.Jet.pt>50)) &(events.Jet.jetId>5)]
        req_jets = (ak.count(event_jet.puId,axis=1) >= 1)
        # event_PV = events.PV[(events.PV.ndof>4)&(events.PV.ndof>)]
        event_level=  req_jets  &req_lep #& req_zmass&req_ghost_removal&req_leppt&req_hmass #& (req_elept | req_mupt)
        # if(len(event_level)>0):event_level = ak.fill_none(event_level,False)        
        # Selected
        selev = events[event_level]    
        output['cutflow'][dataset]['all'] += len(events.Jet)
        # output['cutflow'][dataset]['selected leptons'] += ak.sum(ak.num(events[req_lep],axis=0))
