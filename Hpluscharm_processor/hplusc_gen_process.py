...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        # else :momid=23
        genlep = events.GenPart[((abs(events.GenPart.pdgId) == 11)|(abs(events.GenPart.pdgId) == 13)|(abs(events.GenPart.pdgId) == 15))& (events.GenPart.hasFlags(["fromHardProcess"])==True) & (events.GenPart.hasFlags(["isHardProcess"])==True)]
        
        if '2Nu' in dataset :
            gennu = events.GenPart[((abs(events.GenPart.pdgId) == 12)|(abs(events.GenPart.pdgId) == 14)|(abs(events.GenPart.pdgId) == 16))& (events.GenPart.hasFlags(["fromHardProcess"])==True) & (events.GenPart.hasFlags(["isHardProcess"])==True)]
            gennunu = ak.zip({
        ...
        output['genll_dr'].fill(dataset=dataset,dr=flatten(genlep[:,0].delta_r(genlep[:,1])))
        output['genllc_dr'].fill(dataset=dataset,dr=flatten(genzl.delta_r(genc)))
        output['genl1c_dr'].fill(dataset=dataset,dr=flatten(genlep[:,0].delta_r(genc)))
        output['genl2c_dr'].fill(dataset=dataset,dr=flatten(genlep[:,1].delta_r(genc)))
        output['matched_deepJet'].fill(dataset=dataset,flav=flatten(matchj.hadronFlavour+1*((matchj.partonFlavour == 0 ) & (matchj.hadronFlavour==0))),CvL=flatten(matchj.btagDeepFlavCvL),CvB=flatten(matchj.btagDeepFlavCvB))
        output['matched_deepCSV'].fill(dataset=dataset,flav=flatten(matchj.hadronFlavour+1*((matchj.partonFlavour == 0 ) & (matchj.hadronFlavour==0))),CvL=flatten(matchj.btagDeepCvL),CvB=flatten(matchj.btagDeepCvB))
        
        if '2Nu' in dataset :
            output['gennu_pt'].fill(dataset=dataset,pt=flatten(gennunu.pt))
            output['gennu_phi'].fill(dataset=dataset,phi=flatten(gennunu.phi))
            output['gennuc_dphi'].fill(dataset=dataset,phi=flatten(gennunu.delta_phi(genc)))
            output['gennul_dphi'].fill(dataset=dataset,phi=flatten(gennunu.delta_phi(genlep)))            
            output['gennull_dphi'].fill(dataset=dataset,phi=flatten(gennunu.delta_phi(genzl)))            
            ...
            output['genjj_dr'].fill(dataset=dataset,dr=flatten(genjet[:,0].delta_r(genjet[:,1])))
